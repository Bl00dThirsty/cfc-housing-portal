...
export function TrafficQuality() {
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle className="font-normal text-base">Traffic Quality</CardTitle>
        <CardAction>
          <button
            type="button"
      ...
      <CardContent>
        <div className="h-[270px] w-full pt-2">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ bottom: 10, left: -10, right: 10, top: 10 }}>
              ...
              <Tooltip
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    const actual = payload.find((p) => p.dataKey === "actualQuality")?.value;
                    const baseline = payload.find((p) => p.dataKey === "baselineQuality")?.value;
                    return (
                      <div className="rounded-lg border bg-popover p-2.5 text-xs text-popover-foreground shadow-md space-y-1">
                        <p className="font-semibold">Traffic Quality</p>
                        <div className="flex items-center justify-between gap-4">
                          <span className="text-muted-foreground">Actual:</span>
                          <span className="font-medium font-mono">{Number(actual).toFixed(1)}%</span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                          <span className="text-muted-foreground">Baseline:</span>
                          <span className="font-medium font-mono">{Number(baseline).toFixed(1)}%</span>
                        </div>
                      </div>
                    );
                  }
